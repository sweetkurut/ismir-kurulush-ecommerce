...
export const Card = ({ product }: CardProps) => {
  ...
  const favoriteProductIds = favorites?.results ? favorites.results.map((item: any) => item.product.id) : [];
  ...
  const cartButtonText = !isInStock
    ? "Нет в наличии"
    : isInCart
    ? "В корзине"
    : "В корзину";
    ...
    >
      ...
      <div className={contentClass}>
        <span className={s.category}>
          {product.categories?.[0]?.name ?? "Без категории"}
        </span>
